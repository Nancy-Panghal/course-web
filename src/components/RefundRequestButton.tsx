...
export default function RefundRequestButton({ enrollmentId }: { enrollmentId: string }) {
  ...
  useEffect(() => {
    let cancelled = false
    fetch(`/api/student/refund-request?enrollmentId=${enrollmentId}`)
  ...
  async function submit() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/student/refund-request', {
        method: 'POST',
    ...
    <div style={{ fontSize: 11 }}>
      {!open ? (
        <button onClick={() => setOpen(true)}
          style={{ background: 'none', border: 'none', color: '#52525b', cursor: 'pointer', textDecoration: 'underline' }}>
          Request a refund
        </button>
      ) : result ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end' }}>
          <textarea
            ...
            rows={2}
            style={{ width: 220, fontSize: 11, padding: 6, borderRadius: 6, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }}
          />
