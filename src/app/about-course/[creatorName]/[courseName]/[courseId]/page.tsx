...
import { BrandVarsProvider } from '@/components/BrandVarsContext'
...
export async function generateMetadata({
  ...
  return {
    ...
    // tells Google it is a duplicate of the homepage. Same URL the sitemap lists.
    alternates: {
      canonical: `/about-course/${slugify(course.host_name || 'instructor')}/${slugify(course.name || 'course')}/${courseId}`,
    },
...
export default async function AboutCoursePage({
  ...
  const brandVars = getBrandVars(c)
  ...
  const howItWorksNode = (
    show('howItWorks') && (
      <section className="ak-section py-10 px-6">
        <style>{`
          @keyframes ak-channel-float { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-3px); } }
          .ak-channel-icon { transition: transform 0.2s ease; animation: ak-channel-float 3.2s ease-in-out infinite; }
          .ak-channel-icon:nth-child(2) { animation-delay: 0.4s; }
          .ak-channel-icon:nth-child(3) { animation-delay: 0.8s; }
          .ak-channel-icon:hover { animation-play-state: paused; transform: translateY(-4px) scale(1.08); }
          @media (prefers-reduced-motion: reduce) { .ak-channel-icon { animation: none; } }
        `}</style>
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-5 text-center sm:text-left">
          <div className="flex items-center gap-3 flex-shrink-0">
            {showWhatsappChannel && (
              <span className="ak-channel-icon w-11 h-11 rounded-full flex items-center justify-center" title="WhatsApp"
            ...
            {showTelegramChannel && (
              <span className="ak-channel-icon w-11 h-11 rounded-full flex items-center justify-center" title="Telegram"
                style={{ background: c.pillBg, border: `1px solid ${c.borderSoft}` }}>
                <Send className="w-5 h-5" style={{ color: '#38bdf8' }} />
              </span>
            )}
            <span className="ak-channel-icon w-11 h-11 rounded-full flex items-center justify-center" title="Web"
              style={{ background: c.pillBg, border: `1px solid ${c.borderSoft}` }}>
              <Globe className="w-5 h-5" style={{ color: c.accentText }} />
            </span>
          </div>
          <p className="text-balance" style={{ color: c.textSecondary, fontSize: 'clamp(1rem, 1.2vw, 1.1rem)', lineHeight: 1.6 }}>
            Get instant access on <strong style={{ color: c.textPrimary }}>{deliveryChannelCopy}</strong> — plus full access anytime on the web.
          </p>
        </div>
      </section>
    )
  ...
  return (
    <BrandVarsProvider vars={brandVars}>
    <DraftGate isPublished={course.is_published} courseData={courseData}>
      ...
      />
      <div className="min-h-screen" data-brand-scope style={{ ...brandVars, background: c.bg, color: c.textPrimary, fontFamily: fonts.body }}>
        <style>{`
